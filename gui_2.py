...
st.title("PROJECT: HASAKI RECOMMENDER SYSTEM")

st.sidebar.title("Menu")
menu = st.sidebar.radio("Chọn thông tin:", ["Business problem","Content based RS","Collaborative filtering RS"])
...
if menu=="Business problem":
    ...
    """)  
    st.write("""###### Mục tiêu/vấn đề: Chưa có hệ thống Recommendation System  
        → Xây dựng hệ thống đề xuất để hỗ trợ người dùng nhanh chóng chọn được sản phẩm phù hợp trên Hasaki 
    → Hệ thống sẽ gồm hai mô hình gợi ý chính: 
       - Content-based filtering 
       - Collaborative filtering 
""")
    st.image('hasaki_14.jpg')
    st.image('hasaki_13.jpg')

#Phần 2: Content_based     
elif menu=="Content based RS":
    ...
    # tạo các tab
    tab1, tab2= st.tabs(["Chọn sản phẩm cụ thể", "Chọn theo từ khoá sản phẩm"])
    with tab1:
        ###### Giao diện Streamlit ######
        st.image('hasaki_banner.jpg')

        # Lấy mẫu ngẫu nhiên 5 sản phẩm từ df_products_sorted
        random_products = random.sample(
            [(row['ten_san_pham'], row['ma_san_pham']) for index, row in df_products_sorted.iterrows()],
            k=min(5, len(df_products_sorted))  # Lấy tối đa 5 sản phẩm, hoặc ít hơn nếu danh sách nhỏ hơn 5
        )

        # Tạo dropdown với 5 sản phẩm ngẫu nhiên
        selected_product = st.selectbox(
            "Chọn sản phẩm",
            options=random_products,
    ...
    with tab2:
        ...
        if user_input:
            ...
            if recommendations2.empty:
                ...
                st.image('hasaki_12.jpg')
                
#phần 3: Collaborative filtering
else:
    ...
    # Display
    # Display User ID selection
    user_id = st.selectbox(
        "Chọn User ID",
        options=list(range(1, 11)),  # List User IDs từ 1 đến 10
        format_func=lambda x: f"User {x}",  # Hiển thị dạng 'User X'
        key="user_id_selectbox"  # Thêm key duy nhất
    )

    # Hiển thị danh sách sản phẩm gợi ý sau khi chọn User ID
    ...
    if not matching_rows.empty:
        ...
        if recommendations_product is not None:
            ...
            selected_product = st.selectbox(
                ...
                key="product_selectbox"  # Thêm key duy nhất
            )

            # Hiển thị thông tin sản phẩm ngay khi được chọn
            if selected_product:
